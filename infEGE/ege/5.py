...
for n in range(1, 100):
  # Шаг 1: Преобразуем число N в двоичную строку
  n2 = bin(n)[2:]

  # Шаг 2а: Добавляем первый бит (остаток от суммы цифр по модулю 2)
  if n2.count('1') % 2 == 0:
    n2 = n2 + '0'
  else:
    n2 = n2 + '1'

  # Шаг 2б: Добавляем второй бит (остаток от суммы цифр новой строки по модулю 2)
  if n2.count('1') % 2 == 0:
    n2 = n2 + '0'
  else:
    n2 = n2 + '1'

  # Преобразуем полученную двоичную строку в число R
  R = int(n2, 2)

  # Проверяем, превышает ли R число 63
  if R > 63:
    print(R)  # Выводим минимальное подходящее число R
    break  # Завершаем цикл, так как нашли нужное число
